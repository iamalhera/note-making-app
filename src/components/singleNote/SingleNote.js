...
import { PenTool, Check, Trash } from 'react-feather';
import { Rating } from 'react-simple-star-rating';

const SingleNote = ({ item, refresh}) => {
    ...
    const handleDelete = () => {
        const pass = window.confirm('Are you sure to delete this note ?')
        if (!pass) {
            return
        }
        if (savedData.length) {
    ...
    return (
        <div className="px-2 col-lg-3 col-md-4 col-sm-6 h-100 mb-5 ">
            <div className="card shadow px-2" style={{ backgroundColor: `${item.color.background}` }}>
                ...
                <div className="content">
                    <textarea className='form-control' disabled={!edit} value={content} onChange={(e) => setContent(e.target.value)} style={{ backgroundColor: `${item.color.foreground}` }}>
